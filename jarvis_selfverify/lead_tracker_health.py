#!/usr/bin/env python3
"""BUNDLED lead-tracker health contract (framework + instance, one file).

Generated for drop-in on the Mini. Canonical split version lives in the
jarvis-pwa repo under jarvis_selfverify/. Behaviour is identical.
"""
...
# Mirrors lead_scan.py exactly (verified 2026-07-20 against ~/jarvis/lead_scan.py
# lines 132-140). The token is an authorized_user OAuth token minted with the
# FULL spreadsheets scope, so we must request that same scope or the refresh
# throws a scope-mismatch error. Read-only here is a BEHAVIORAL guarantee: this
# module only ever calls values().get / spreadsheets().get -- never update,
...
def _load_credentials(cred_path: str):
    ...
    if not os.path.exists(cred_path):
        raise RuntimeError(
            "Credential not found at {}. Set JARVIS_GOOGLE_CRED.".format(cred_path))
